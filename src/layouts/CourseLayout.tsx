        ...
        <div className="flex flex-col md:flex-row min-h-screen bg-slate-50">
            ...
            `}>
                <div className="p-6 border-b border-slate-100 hidden md:block select-none">
                    <h2 className="text-xl font-bold text-slate-900 tracking-tight">CLI Mastery</h2>
                    <p className="text-xs text-slate-500 mt-1">Foundations to Advanced</p>
                </div>

                <div className="flex-1 p-4 space-y-8">
                    {/* Spacer for mobile header */}<div className="md:h-12 md:hidden"></div>

                    <div>
                        <h3 className="px-2 text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Basics</h3>
                        <ul className="space-y-1">
                            {courseContent.filter(t => t.level === 'Basic').map(topic => (
                                <li key={topic.id}>
                                    <Link
                                        to={`/course/${topic.id}`}
                                    ...
                                    >
                                        <div className="flex items-center">
                                            <span className="mr-2 opacity-50 text-xs">#{topic.id.split('-')[0] || '•'}</span>
                    ...
                    <div>
                        <h3 className="px-2 text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Advanced</h3>
                        <ul className="space-y-1">
                            {courseContent.filter(t => t.level === 'Advanced').map(topic => (
                                <li key={topic.id}>
                                    <Link
                                        to={`/course/${topic.id}`}
                                        onClick={handleLinkClick}
